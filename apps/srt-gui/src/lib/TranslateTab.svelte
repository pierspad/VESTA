<script lang="ts">
  ...
  let availableModels = $derived.by(() => {
    ...
    if (selectedProviderFamily === "custom") {
      return getModelsForProvider("custom");
    }
    return [];
  });

  // Effective model: use customModelId if filled, otherwise selectedModel from dropdown
  let effectiveModel = $derived(customModelId.trim() || selectedModel);
  ...
  let hasValidKey = $derived.by(() => {
    ...
    if (selectedProviderFamily === "custom") {
      return apiKeys.some(
        (k) => k.apiType === "custom" && k.apiUrl && k.apiUrl.trim().length > 0,
      );
    }
    return false;
  });
  ...
  async function startTranslation() {
    ...
    if (selectedProviderFamily === "local") {
      ...
      const customKey = apiKeys.find((k) => k.apiType === "custom");
      if (customKey) {
        apiUrl = customKey.apiUrl || null;
        if (customKey.apiKey && customKey.apiKey.trim()) {
      ...
      <div class="glass-card p-5">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <button
                ...
                class="flex items-center gap-2 p-2.5 rounded-lg transition-all duration-200 border text-left text-xs flex-1 min-w-0
          ...
          <div>
            ...
            >
            {#if !customModelId.trim() && availableModels.length > 0}
              <SearchableSelect
            ...
            {#if customModelId.trim()}
              ...
              <p class="text-[10px] text-emerald-400 mt-1">
                ...
                <strong>{customModelId.trim()}</strong>
              </p>
            {:else}
              <button
                type="button"
                onclick={() => (customModelId = " ")}
                class="text-[10px] text-gray-500 hover:text-cyan-400 mt-1.5 transition-colors flex items-center gap-1"
              >
                <svg
                  class="w-3 h-3"
                  fill="none"
                  ...
                  /></svg
                >
                {t("translate.customModelPlaceholder")}
              </button>
            {/if}
          </div>
          <div
